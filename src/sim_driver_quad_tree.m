...
function S = initResultsIO(S)
    ...
    S.paths.catalog = fullfile(S.paths.run, 'catalog.mat');       % v7.3
    S.paths.sol_dir = fullfile(S.paths.run, 'solutions');         % optional
    if ~exist(S.paths.run,'dir'), mkdir(S.paths.run); end
    if ~exist(S.paths.sol_dir,'dir'), mkdir(S.paths.sol_dir); end

    % initialize catalog (table) if new
    if ~exist(S.paths.catalog,'file')
        T = table( ...
            zeros(0,1), zeros(0,1), ...          % H0_1, H0_2
            zeros(0,1,'int16'), ...              % label
            zeros(0,1), zeros(0,1), ...          % E_total, P_osm
            zeros(0,1), zeros(0,1), ...          % BCmax, DEmax
            zeros(0,1,'uint16'), ...             % mesh_points
            strings(0,1), ...                    % attempt_label
            strings(0,1), ...                    % sol_key (optional)
            'VariableNames', {'H0_1','H0_2','label','E_total','P_osm', ...
                              'BCmax','DEmax','mesh','attempt','sol_key'});
...
function processQuadtree(S, QT, params, cache)
    ...
    while ~isempty(QT.queue) && cellCount < maxCells
        ...
        logmsg(S, 'Progress: Accepted Cells: %i (max = %i).', cellCount, maxCells);
        ...
        try
            ...
            if isBCOK && isDEOK && isGeomOK
                ...
                try
                    ...
                    delta_used  = attempts(a).delta; %#ok<NASGU>
                    solver_opts = attempts(a).opts;  %#ok<NASGU>
                    BCmax_save  = BCmax;             %#ok<NASGU>
                    DEmax_save  = DEmax;             %#ok<NASGU>
...
function writeCatalogRow(S, hash, label, E, P, bcmax, demax, mesh)
    row = {datestr(now,'yyyy-mm-dd HH:MM:SS'), hash, ...
