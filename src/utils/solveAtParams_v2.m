...
% and optional homotopy recording via sim.SP.SaveHomotopy (true/false).
    ...
    say('\n=== solveAtParams: starting solve at H0 = [%+.3f, %+.3f] ===', Htgt(1), Htgt(2));
    ...
    stepCaps = [Inf, 0.50, 0.25, 0.12, 0.06, 0.03];  % ladder of H0 step caps
    if isfield(TH,'delta_list') && ~isempty(TH.delta_list)
        deltas = TH.delta_list(:)';
    else
        % default: baseline, slightly larger, and smaller
        deltas = unique([delta0, min(2*delta0,0.02), max(0.5*delta0,5e-3)], 'stable');
    end
    ...
    for rung = 1:numel(stepCaps)
        ...
        if isfinite(stepMax), say('  continuation path: up to step %.3g', stepMax);
        else,                 say('  direct attempt at target H0'); end
                ...
                while true
                    ...
                    try
                        ...
                        if saveHomotopy
                            ...
                            homolog(end+1) = struct('H0',next,'delta',usedPar.delta, ...
                                                    'mesh',numel(curSol.x),'BCmax',BCi,'DEmax',DEi); %#ok<AGROW>
                        end
    ...
    [BCmax,~]   = bc_diagnostics(sol, @(ya,yb,lam) BendV_Lag_EIGp_BC_impl(ya,yb,lam,usedPar));
    [DEmax,~,~] = de_residual(sol, @(s_,y_,lam_) BendV_Lag_EIGp_DE_impl(s_,y_,lam_,usedPar));
